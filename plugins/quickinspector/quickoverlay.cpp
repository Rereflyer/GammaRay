...
public:
    ...
    {
        static QHash<QLatin1String, QQuickItemPropertyCache> s_cache;
        const auto meta = item->metaObject();
        const auto className = QLatin1String(meta->className());
        const auto it = s_cache.constFind(className);
        if (it != s_cache.cend())
            return *it;
        else
            return *s_cache.insert(className, QQuickItemPropertyCache(meta));
    }
